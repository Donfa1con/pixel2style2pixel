    ...
    def setup(self):
        ...
        for key in model_paths.keys():
            if key in ["ffhq_frontalize", "toonify"]:
                ...
                self.transforms[key] = transforms.Compose([transforms.Resize((256, 256)), transforms.ToTensor()])
    ...
    def predict(self, model, image):
        ...
        if model == "celebs_super_resolution":
            res = np.concatenate(
                [
                    np.array(
                        input_vis_image.resize((self.opts[model]["output_size"], self.opts[model]["output_size"]))
                    ),
                    np.array(output_image.resize((self.opts[model]["output_size"], self.opts[model]["output_size"]))),
                ],
                axis=1,
            )
        else:
